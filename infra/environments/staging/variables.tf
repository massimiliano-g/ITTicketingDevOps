variable "unique_suffix" {
  description = "Suffisso alfanumerico minuscolo (senza trattini) per rendere globalmente unici i nomi di Key Vault e SQL Server. Consigliato: lo stesso suffisso random a 6 caratteri usato per gli altri ambienti — il nome completo differisce comunque per via del segmento '-staging-'."
  type        = string
...
variable "aad_admin_login_name" {
  description = "UPN/email dell'utente Entra ID che sarà amministratore del server SQL di staging."
  type        = string
}
